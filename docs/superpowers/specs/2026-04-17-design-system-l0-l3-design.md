...
**타깃**: **WCAG 2.1 Level AA**. 이 수준을 모든 프리미티브·페이지의 기본 기준선으로 삼는다. 이유: 표준 수준·해외 대부분 앱 준수·공공기관 요구와도 호환. Level AAA는 소곤 규모에 과함(오버킬).

구체적으로:
- **색 대비**: 본문 텍스트 vs 배경 최소 4.5:1, large text(18px+) 3:1. 모든 L2 시맨틱 컬러 쌍에 대해 라이트·다크 모두 검증(플랜 실행 단계에서 자동 체크).
- **시맨틱 요소 우선** (`<button>`, `<input>`, `<dialog>`) — `<div>` 인터랙티브 금지.
- **키보드 조작** (Tab / Shift+Tab / Enter / Space / Escape / 화살표) 가능.
- **`focus-visible` 스타일**은 `--color-border-focus` 사용. 마우스 클릭엔 링 안 보이고 키보드 포커스에만 보이게.
- **ARIA는 필요 최소** (`aria-label`, `aria-describedby`, `aria-invalid`, `aria-busy`, `aria-live`).
- **Modal**: focus trap, Escape 닫기, 배경 스크롤 잠금, 복귀 포커스.
- **motion**: `prefers-reduced-motion` 존중 (애니메이션 해제 또는 opacity fade로 축소).
...
7. 모든 L2 시맨틱 컬러 쌍(라이트/다크 각각)이 **WCAG 2.1 AA 대비율** 을 충족 (본문 4.5:1, large text 3:1). 실패 쌍은 팔레트 조정 후 재검증.
